...
The less the nesting, the more linear the code is, and the easier it becomes to follow. One level of nesting depth raises complexity and thus _[cognitive load](https://en.wikipedia.org/wiki/Cognitive_load)_ in reasoning about the behavior of the code.

## Small Functions: Modularization and Reusability

[Robert C. Martin](https://en.wikipedia.org/wiki/Robert_C._Martin), better known as Uncle Bob, is insistent on small functions that do one thing and do it well. He's also credited to the well-known [SOLID](https://en.wikipedia.org/wiki/SOLID) principles. Small functions support modularity in that they encapsulate some specific behavior or operation. Ideally, every function should have a single task to compute a value, format data, or deal with some aspect of the business logic. Also, in the realm of functional programming, pure functions can [decrease in thousands the possibility of bugs](https://news.ycombinator.com/item?id=9758600) to occur due to racing conditions or accessing global state.
